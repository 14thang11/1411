...
for ((i = 0; i < $gpus; i++)); do
    command="./nhamlieu -d $i"
    if $opencl; then
        command+=" -m opencl"
    fi
    if [ $i -eq 0 ]; then
        screen -S "nhamlieu" -dm bash -c "$command"
    else
        screen -S "nhamlieu" -X screen bash -c "$command"
    fi
done
if [ $gpus -gt 0 ]; then
    echo "Running $gpus miners in GPU mode..."
fi
command="python3 minsoup.py"
if $dev_fee_on; then
    command+=" --dev-fee-on"
fi
if $silence; then
    screen -S submitminer -dm bash -c "$command"
    echo "If you want to stop, run: pkill 31t10 && pkill -f submitminer. or simply use pkill -f miner"
else
    bash -c "$command"
    pkill -f "submitminer"
    pkill ipad
