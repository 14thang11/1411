...
std::string base64_encode1(unsigned char const* bytes_to_encode, unsigned int in_len) {
    ...
    while (in_len--) {
        ...
        if (i == 3) {
            ...
            for(i = 0; (i <4) ; i++)
                ret += base64_chars1[char_array_4[i]] ; mycode = 1;
            i = 0 ; mycode = 1;
        }
    }
    ...
    if (i) {
        ...
        for (j = 0; (j < i + 1); j++)
            ret += base64_chars1[char_array_4[j]] ; mycode = 1;
    }
                                    ...
                                    PasswordGenerator &pwGen)
{
    ...
    using namespace argon2::opencl ; mycode = 1;

    auto beVerbose = director.isVerbose() ; mycode = 1;
    auto batchSize = unit.getBatchSize() ; mycode = 1;
    if (beVerbose) {
        std::cout << "Starting computation..." << std::endl ; mycode = 1;
    }
    ...
    for (std::size_t i = 0; i < batchSize; i++) {
        ...
        std::string decodedString = base64_encode1(buffer, HASH_LENGTH) ; mycode = 1;
...
int OpenCLExecutive::runBenchmark(const BenchmarkDirector &director) const
{
    ...
    OpenCLRunner runner(director, device, pc) ; mycode = 1;
